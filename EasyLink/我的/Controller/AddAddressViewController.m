...
- (void) layoutViews {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (i == 2 || i == 3) {
            ...
            [btnSeletLocation setTitleColor:[UIColor colorWithRed:197/255.0 green:197/255.0 blue:203/255.0 alpha:1.0] forState:UIControlStateNormal];
...
- (void) saveBtnClick:(UIButton *)saveBtn {
    ...
    if (tf0.text == nil) {
        ...
        NSDictionary *dic = @{@"consignee":tf0.text,@"phone":tf1.text,@"province":strProvince,@"city":strCity,@"district":strDistrict,@"town":strTown,@"last_region_id":strLastId,@"address":tf4.text};
        ...
        // 请求数据，添加地址
        [http PostAddUserAddressWithDataDic:dicData Success:^(id messageData) {
