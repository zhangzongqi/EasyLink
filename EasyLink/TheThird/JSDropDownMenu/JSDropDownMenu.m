...
// 按钮的背景色
//#define BtnBackColor [UIColor colorWithRed:113/255.0 green:102/255.0 blue:245/255.0 alpha:1.0]
#define BtnBackColor [UIColor clearColor]
...
- (void)setDataSource:(id<JSDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        // 此处用于修改三个按钮的背景色
        CALayer *bgLayer = [self createBgLayerWithColor:BtnBackColor andPosition:bgLayerPosition];
        [self.layer addSublayer:bgLayer];
        [tempBgLayers addObject:bgLayer];
        //title
        CGPoint titlePosition = CGPointMake( (i * 2 + 1) * textLayerInterval , self.frame.size.height / 2);
        NSString *titleString = [_dataSource menu:self titleForColumn:i];
        CATextLayer *title = [self createTextLayerWithNSString:titleString withColor:FUIColorFromRGB(0xffffff) andPosition:titlePosition];
    ...
    // 此处是设置底边阴影的颜色
    _bottomShadow.backgroundColor = [UIColor clearColor];
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        //self tapped
//        self.backgroundColor = [UIColor colorWithRed:144/255.0 green:137/255.0 blue:249/255.0 alpha:1.0];
        
        // 新闻条渐变背景色
        UIColor *topColor = FUIColorFromRGB(0x7d6dfa);
        UIColor *bottomColor = FUIColorFromRGB(0x7361f8);
        UIImage *bgImg = [UIImage gradientColorImageFromColors:@[topColor, bottomColor] gradientType:GradientTypeTopToBottom imgSize:CGSizeMake(self.frame.size.width, self.frame.size.height)];
...
- (CAShapeLayer *)createSeparatorLineWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    [path addLineToPoint:CGPointMake(160, self.frame.size.height/2)]; // 此处是竖着的分割线高度
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        if (i != tapIndex) {
            ...
            // 此处用于改变未选中按钮的颜色
            [(CALayer *)self.bgLayers[i] setBackgroundColor:BtnBackColor.CGColor];
        }
    }
    ...
    if (displayByCollectionView) {
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            // 此处用于改变选中按钮的颜色
            [(CALayer *)self.bgLayers[tapIndex] setBackgroundColor:BtnBackColor.CGColor];
        } else {
...
- (void)backgroundTapped:(UITapGestureRecognizer *)paramSender
{
    ...
    //
    [(CALayer *)self.bgLayers[_currentSelectedMenudIndex] setBackgroundColor:BtnBackColor.CGColor];
}
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if (leftOrRight == 1) {
        ...
        CGFloat marginX = (self.frame.size.width * 2/3 - textSize.width) / 2;
        
        titleLabel.frame = CGRectMake(marginX, 0, textSize.width, cell.frame.size.height);
//        titleLabel.centerX = cell.frame.size.width / 2;
//        titleLabel.centerY = cell.frame.size.height / 2;
        //右边tableview
...
//            accessoryImageView.frame = CGRectMake(titleLabel.frame.origin.x+titleLabel.frame.size.width+10, (self.frame.size.height-12)/2, 16, 12);
            accessoryImageView.frame = CGRectMake(titleLabel.frame.origin.x - 16 - 10, (self.frame.size.height-12)/2, 16, 12);
...
// 自己加的方法，用于改变选择器上的文字
- (void) makemenuIndex:(NSInteger) needChangeIndex andTitle:(NSString *)strTitle {
    
    CATextLayer *title = (CATextLayer *)_titles[needChangeIndex];
    title.string = strTitle;
    
    [self animateIdicator:_indicators[needChangeIndex] background:_backGroundView leftTableView:_leftTableView rightTableView:_rightTableView title:_titles[needChangeIndex] forward:NO complecte:^{
        _show = NO;
    }];
    [(CALayer *)self.bgLayers[needChangeIndex] setBackgroundColor:BtnBackColor.CGColor];
    
    CAShapeLayer *indicator = (CAShapeLayer *)_indicators[needChangeIndex];
