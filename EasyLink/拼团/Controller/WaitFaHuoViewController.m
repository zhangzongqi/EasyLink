...
#import "WaitFaHuoViewController.h"
...
#import "pingtuanStatusView.h" // 订单状态

@interface WaitFaHuoViewController ()<UICollectionViewDataSource,UICollectionViewDelegate> {
...
@property (nonatomic, copy) pingtuanStatusView *pingtuanStatusVc;
...
@property (nonatomic, copy) UIButton *sureReceiveBtn;

@end

@implementation WaitFaHuoViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.self.view.backgroundColor = FUIColorFromRGB(0xffffff);
...
- (void) createUI {
    ...
    _numAndstatusVc.dingdanStatusLb.text = @"待发货";
    
    // 订单状态和送达预计
    _pingtuanStatusVc = [[pingtuanStatusView alloc] initWithFrame:CGRectMake(0, 0, W, W * 0.240625)];
    [_bigScrollView addSubview:_pingtuanStatusVc];
    [_pingtuanStatusVc mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_numAndstatusVc.mas_bottom).with.offset(1);
        make.left.equalTo(self.view);
        make.width.equalTo(@(W));
        make.height.equalTo(@(W * 0.240625));
    }];
    _pingtuanStatusVc.backgroundColor = FUIColorFromRGB(0xffffff);
    _pingtuanStatusVc.statusLb.text = @"拼团成功,准备配送中";
    _pingtuanStatusVc.songDaTimeLb.text = @"预计送达时间:2017.2.22 下午18:00";
    ...
    [_addressVc mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_pingtuanStatusVc.mas_bottom).with.offset(W / 64);
        make.left.equalTo(self.view);
        make.width.equalTo(@(W));
    ...
    // 立即支付
    _sureReceiveBtn = [[UIButton alloc] init];
    [_bottomView addSubview:_sureReceiveBtn];
    [_sureReceiveBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(_bottomView);
        make.right.equalTo(_bottomView).with.offset(- W * 0.046875);
        make.width.equalTo(@(W * 0.3125));
        make.height.equalTo(@(W * 0.084375));
    }];
    _sureReceiveBtn.layer.cornerRadius = W * 0.084375/2;
    _sureReceiveBtn.clipsToBounds = YES;
    _sureReceiveBtn.layer.borderColor = [FUIColorFromRGB(0xbcb2ff) CGColor];
    _sureReceiveBtn.layer.borderWidth = 1.0;
    [_sureReceiveBtn setTitleColor:FUIColorFromRGB(0x8979ff) forState:UIControlStateNormal];
    [_sureReceiveBtn setTitle:@"确认收货" forState:UIControlStateNormal];
    if (iPhone6SP) {
        _sureReceiveBtn.titleLabel.font = [UIFont systemFontOfSize:16];
    }else if (iPhone6S) {
        _sureReceiveBtn.titleLabel.font = [UIFont systemFontOfSize:15];
    }else {
        _sureReceiveBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    }
