...
@interface ZhiFuViewController ()<UICollectionViewDataSource,UICollectionViewDelegate> {
...
@property (nonatomic, copy) DingDanBianhaoAndStatus *statusVc;
...
- (void) initData {
    ...
    _titleLabelArr = @[@"支付方式",@"下单时间"];
    _infoArr = @[@"微信支付",@"17-02-17 10:48:45"];
}
...
- (void) createUI {
    ...
    topTipLb.text = @"请尽快完成支付,24小时后订单自动取消。";
    if (iPhone6SP) {
    ...
    _statusVc = [[DingDanBianhaoAndStatus alloc] initWithFrame:CGRectMake(0, 0, W, H * 0.04)];
    [_bigScrollView addSubview:_statusVc];
    [_statusVc mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(topTipLbView.mas_bottom);
        make.left.equalTo(self.view);
        make.width.equalTo(@(W));
        make.height.equalTo(@(W * 0.09375));
    }];
    _statusVc.dingDanNumLb.text = @"订单编号:1719119057";
    _statusVc.dingdanStatusLb.text = @"待付款";
    ...
    [_addressVc mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_statusVc.mas_bottom).with.offset(1);
        make.left.equalTo(self.view);
        make.width.equalTo(@(W));
    ...
    [_payBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.equalTo(@((W - W * 0.046875 * 3) / 2));
        make.height.equalTo(@(W * 0.084375));
    }];
    _payBtn.layer.cornerRadius = W * 0.084375/2;
    _payBtn.clipsToBounds = YES;
    _payBtn.backgroundColor = FUIColorFromRGB(0x8979ff);
    [_payBtn setTitleColor:FUIColorFromRGB(0xffffff) forState:UIControlStateNormal];
    [_payBtn setTitle:@"立即支付" forState:UIControlStateNormal];
    // 取消订单
    _cancleBtn = [[UIButton alloc] init];
    [_bottomView addSubview:_cancleBtn];
    [_cancleBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_bottomView).with.offset(W * 0.046875);
        make.centerY.equalTo(_bottomView);
        make.width.equalTo(@((W - W * 0.046875 * 3) / 2));
        make.height.equalTo(@(W * 0.084375));
    }];
    _cancleBtn.layer.cornerRadius = W * 0.084375/2;
    _cancleBtn.clipsToBounds = YES;
    _cancleBtn.layer.borderColor = FUIColorFromRGB(0x8979ff).CGColor;
    _cancleBtn.layer.borderWidth = 1.0;
    [_cancleBtn setTitleColor:FUIColorFromRGB(0x8979ff) forState:UIControlStateNormal];
